...
let type_ ~hide ~loc return_type ty =
  ...
  [%type: [%t ty] -> [%t hty] -> [%t return_type ~loc]]

let sig_type_decl return_type deriver_label ~ctxt (_rec_flag, tds) =
  let hide = not (Expansion_context.Deriver.inline ctxt) in
  let tds = List.map tds ~f:name_type_params_in_td in
  List.map tds ~f:(fun td ->
    let compare_of = combinator_type_of_type_declaration td ~f:(type_ ~hide return_type) in
...
let return_type ~loc = [%type: bool] in 
let deriver_label = "equal" in
Deriving.add deriver_label ~sig_type_decl:(Deriving.Generator.V2.make_noarg (sig_type_decl return_type deriver_label)) ;;

let return_type ~loc = [%type: int]in 
let deriver_label = "compare" in
Deriving.add deriver_label ~sig_type_decl:(Deriving.Generator.V2.make_noarg (sig_type_decl return_type deriver_label)) ;;
